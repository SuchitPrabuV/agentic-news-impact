...
DATA_PATH = Path(__file__).resolve().parents[2] / "data.csv"
...
def generate_samples(n_per_sector=75):
    ...
    for sector, keywords in SECTORS.items():
        for _ in range(n_per_sector):
            ...
            data.append({"text": text, "label": sector})

    return data


def save_csv(data, filename: Path = DATA_PATH):
    with open(filename, "w", newline="", encoding="utf-8") as file_handle:
        writer = csv.DictWriter(file_handle, fieldnames=["text", "label"])
        writer.writeheader()
        writer.writerows(data)


def main():
    data = generate_samples(n_per_sector=75)
